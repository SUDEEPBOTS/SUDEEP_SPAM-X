...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» ™°‌ᯓ𓂃❛ 𝗦 𝛖 𝛅 ֟፝ᥱ 𝛆 𝛒 </𝟑 𝁘ໍ𝀛𓂃🍷", parse_mode=None, link_preview=None)
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"💫🥀 🫧 🇸⎯꯭‌ ༓‌🗡⃪꯭꯭‌𝐍꯭𝛆‌꯭፝‌𝐗\n» {mp} 𝙼𝚂")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        try:
            ...
            await ok.edit("» ᴀʙᴇ ɢᴀɴᴅᴜ....ᴜsᴇʀ ᴘᴇ ʀᴇᴘʟʏ ᴋᴀʀ...!")
        if len(sudousers) > 0:
            newsudo = f"{sudousers} {target}"
        else:
            newsudo = f"{target}"
        await ok.edit(f"» **ɴᴇᴡ sᴛʀᴀɴɢᴇʀ's ʙᴇᴛᴀ**: `{target}`\n» ʙᴏᴛ ғɪʀ sᴇ sᴜʀᴜ ʜᴏ ʀʜᴀ ..!")
